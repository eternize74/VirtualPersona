...
> **Phase 1 MVP + Phase 2 고도화 + Phase 3 진행 중** ✅🚧
...
| 감정 프리셋 (6종) | 2 | ✅ |
| 확장 렌더러 (눈/입 스타일) | 2 | ✅ |
| 손 추적 (MediaPipe GestureRecognizer) | 2 | ✅ |
| 제스처 인식 및 전송 (7종) | 2 | ✅ |
| WebGPU 인프라 구축 | 3-1 | ✅ |
| GPU 모드 선택 / 성능 모니터 | 3-1 | ✅ |
| LivePortrait 모델 통합 | 3-2 | 🚧 |
...
### Phase 1 – MVP ✅ 완료
- [x] WebRTC 1:1 연결
- [x] 얼굴 추적 (MediaPipe FaceLandmarker)
- [x] 2D 아바타 렌더 (Canvas)
- [x] 파라미터 전송 (DataChannel)

### Phase 2 – 고도화 ✅ 완료
- [x] 아바타 커스터마이징 (색상, 눈/입 스타일)
- [x] 감정 프리셋 6종 (기쁨, 슬픔, 놀람, 화남, 윙크, 사랑)
- [x] 카메라로 돌아가기 기능 (ESC 키, 버튼)
- [x] 확장 렌더러 (AvatarRendererV2)
- [x] Phase 2 전용 라우팅 (/home-v2, /room-v2)
- [x] 손 추적 및 제스처 인식 (7종)

### Phase 3 – Neural Avatar 🚧 진행 중
- [x] **Phase 3-1**: WebGPU 인프라 구축
  - [x] GPU 감지 및 모드 선택 (useWebGPU)
  - [x] 성능 모니터링 (PerformanceMonitor)
  - [x] 테스트 페이지 (/test-v3)
- [ ] **Phase 3-2**: LivePortrait 모델 통합
  - [ ] ONNX 변환 및 FP16 양자화
  - [ ] 브라우저 모델 로딩
- [ ] **Phase 3-3**: 최적화
  - [ ] 성능 fallback 구조
...
│   │   │   ├── test/               # 테스트 - Phase 1
│   │   │   ├── home-v2/            # 홈 - Phase 2
│   │   │   ├── room-v2/[roomId]/   # 통화 룸 - Phase 2
│   │   │   └── test-v2/            # 테스트 - Phase 2
│   │   ├── components/             # UI 컴포넌트
│   │   │   ├── AvatarSelector.tsx  # 아바타 선택 그리드
│   │   │   ├── AvatarRenderer.tsx  # Canvas 2D 렌더러 - Phase 1
│   │   │   ├── AvatarRendererV2.tsx # 확장 렌더러 - Phase 2
│   │   │   ├── AvatarCustomizer.tsx # 커스터마이징 UI - Phase 2
│   │   │   ├── EmotionPresets.tsx  # 감정 프리셋 - Phase 2
│   │   │   └── ConnectionStatus.tsx # 연결 상태 표시
│   │   ├── hooks/                  # 커스텀 훅
│   │   │   ├── useFaceTracking.ts  # MediaPipe 얼굴 추적
│   │   │   └── useWebRTC.ts        # P2P DataChannel 통신
│   │   ├── types/
│   │   │   ├── avatar.ts           # 타입 정의 - Phase 1
│   │   │   └── avatarV2.ts         # 확장 타입 - Phase 2
...
#### Phase 2 (고도화 기능)
1. 브라우저에서 http://localhost:3000/home-v2 접속
2. 🎨 버튼으로 아바타 커스터마이징 (색상, 스타일)
3. "새 대화 시작" 클릭하여 Phase 2 룸 입장
4. 통화 중 1-6 키 또는 감정 버튼으로 감정 전송
5. ESC 키 또는 "📹 카메라로" 버튼으로 원래 상태 복귀
...
**Phase 2 테스트 페이지:**
- http://localhost:3000/test-v2 에서 커스터마이징/감정 프리셋 테스트
